...
def status(fyre_user_name: str, fyre_api_key: str, cluster_name: str, json: bool, site: Optional[str]):
    """List the status of an OCP+ cluster"""

    dg.utils.network.disable_insecure_request_warning()
    OCPPlusAPIManager(fyre_user_name, fyre_api_key).get_cluster_status(cluster_name, site).format(json)
