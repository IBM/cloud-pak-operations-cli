...
def get_quick_burn_sizes(fyre_user_name: str, fyre_api_key: str, json: bool, site: Optional[str]):
    """Get available sizes for a quick burn deployment"""

    dg.utils.network.disable_insecure_request_warning()
    OCPPlusAPIManager(fyre_user_name, fyre_api_key).get_quickburn_sizes(site).format(json)
