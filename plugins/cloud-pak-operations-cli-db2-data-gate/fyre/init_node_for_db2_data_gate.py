...
async def _compile_selinux_policy_module(remote_client: cpo.utils.ssh.RemoteClient, node: str):
    await remote_client.execute(f"ssh core@{node} sudo checkmodule -m --mls --output db2u-nfs.mod db2u-nfs.te")


async def _copy_type_enforcement_file_to_infrastructure_node(remote_client: cpo.utils.ssh.RemoteClient):
    await remote_client.upload(cpo.config.configuration_manager.get_deps_directory_path() / "db2u-nfs" / "db2u-nfs.te")


async def _copy_type_enforcement_file_to_worker_node(remote_client: cpo.utils.ssh.RemoteClient, node: str):
    await remote_client.execute(f"scp db2u-nfs.te core@{node}:")


async def _create_selinux_policy_module_package(remote_client: cpo.utils.ssh.RemoteClient, node: str):
...
async def _install_selinux_policy_module_package(remote_client: cpo.utils.ssh.RemoteClient, node: str):
